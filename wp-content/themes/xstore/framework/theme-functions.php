...
if(!function_exists('etheme_add_body_classes')) {
    function etheme_add_body_classes($classes) {
        ...
        $classes[] = (etheme_get_option('header_overlap')) ? 'et-header-overlap' : 'et-header-not-overlap';
...
if(!function_exists('etheme_page_background')) {
    function etheme_page_background() {
        ...
        $bg_image = etheme_get_custom_field('bg_image');
        $bg_color = etheme_get_custom_field('bg_color');
...
if(!function_exists('etheme_get_logo_data')) {
    function etheme_get_logo_data() {
        ...
        $return['logo']['size'] = @getimagesize($return['logo']['src']);
        $return['fixed_logo']['size'] = @getimagesize($return['fixed_logo']['src']);
            ...
            if ( is_user_logged_in() ) {
                if( class_exists('WooCommerce')) {
                    ...
                    foreach ( wc_get_account_menu_items() as $endpoint => $label ) {
                        $links['my-account']['submenu'] .= '<li class="' . wc_get_account_menu_item_classes( $endpoint ) . '"><a href="' . esc_url( wc_get_endpoint_url( $endpoint, '', $permalink ) ) . '">' . esc_html( $label ) . '</a></li>';
                    }
