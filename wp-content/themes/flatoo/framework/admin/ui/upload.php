    ...
    echo '<div ' . build_section_class( 'section-upload', $item ) . '>';
        ...
        echo '<div class="controls">';
            echo '<div class="uploader clearfix">';
                ...
                printf( '<input class="upload_button button-secondary" type="button" value="%2$s" id="upload_%1$s" />', esc_attr( $id ), __( 'Upload', 'spyropress' ) );
                if ( is_array( @getimagesize( $value ) ) ) {
                    ...
                    if ( '' != $value ) {
                        ...
                        if ( $image ) {
                            print '<img src="' . esc_url( $value ) . '" alt="" />' . $remove;
                        }
                        else {
                            $parts = explode( '/', $value );
...
function spyropress_ui_gallery( $item, $id, $value, $is_widget = false, $is_builder = false ) {
    ...
    echo '<div ' . build_section_class( 'section-gallery section-full', $item ) . '>';
        ...
        printf( '<input class="gallery_upload_button button button-primary button-large right" type="button" value="%2$s" id="upload_%1$s" />', esc_attr( $id ), __( 'Add/Edit Gallery', 'spyropress' ) );
        build_heading( $item, $is_widget );
        build_description( $item );
        echo '<div class="controls">';
